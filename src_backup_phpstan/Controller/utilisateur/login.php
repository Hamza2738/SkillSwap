...
public function inscription(
    ...
    if ($request->isMethod('POST')) {
        ...
        $utilisateur->setNom((string) $request->request->get('nom'));
        $utilisateur->setPrenom((string) $request->request->get('prenom'));
        $utilisateur->setEmail((string) $request->request->get('email'));
        $utilisateur->setTelephone((string) $request->request->get('telephone'));
        $utilisateur->setRole((string) $request->request->get('role', 'freelance'));
        $utilisateur->setStatut('actif');

        // ❌ SANS HASH (mot de passe direct)
        $utilisateur->setMotDePasse((string) $request->request->get('mot_de_passe'));
...
public function frontCompetences(
    ...
    if ($form->isSubmitted() && $form->isValid()) {
        $competence->setEmail((string) $user->getEmail());
